...
```
Business bd/
├── config.py               # 全局配置（路径、停用词、过滤规则、列定义）
├── utils.py                # 通用工具（HTTP、JSON、CSV、字符串处理）
├── scripts/                # 所有独立脚本
│   ├── parse_abn.py                   # XML → CSV 解析
│   ├── filter_records.py              # 过滤 + 名称处理
│   ├── merge_update.py                # 增量合并
│   ├── merge_add.py                   # 手工补充合并
│   ├── clean_keywords.py              # 关键词清洗
│   ├── categorize.py                  # 规则分类
│   ├── category_rules.py              # 分类规则数据
│   ├── merchant_classifier.py         # AI 分类（DeepSeek）
│   └── verify_third_party_merchants.py # AI 第三方验证（DeepSeek）
├── xml2csv.sh               # 工作流 A 一键执行
├── clean_keywords.sh        # 关键词清洗（直接对 merchant_kb.csv）
├── run_full_pipeline.sh     # 工作流 B 一键执行
...
```
raw/*.xml  →  parse_abn  →  filter_records  →  merge_update  →  clean_keywords  →  categorize  →  merchant_kb.csv
```

| 步骤 | 脚本 | 功能 |
|------|------|------|
| 1 | `parse_abn.py` | 解析 `raw/*.xml` 中的 ABR 记录，提取全部字段，输出 `data/parsed/{filename}.csv` |
| 2 | `filter_records.py` | 过滤（只保留 PRV/PUB 实体类型，排除 2023 年前注销的），生成 merchant_name 和 keywords，输出 `data/filtered.csv` |
| 3 | `merge_update.py` | 将新的 `filtered.csv` 与已有 `data/kb_internal.csv` 按 match_key 做增量合并，记录变更日志 |
| 4 | `clean_keywords.py` | 清洗 keywords 字段：移除过短词、停用词、大小写去重 |
| 5 | `categorize.py` | 基于关键词规则将商户分类到 24 个行业类别 |
| — | `merge_add.py` | 手动补充：将 `add/*.csv` 中的商户合并到最终知识库 |
...
将 `raw/` 目录下的所有 ABR XML 文件解析为 CSV，输出到 `data/parsed/`。每条记录提取 ABN、实体类型、名称、地址、状态等 20 个字段。

### filter_records — 过滤与名称处理

```bash
python scripts/filter_records.py
```

读取 `data/parsed/*.csv`，执行两项过滤：
- 只保留 `EntityTypeInd` 为 `PRV`（私有公司）或 `PUB`（上市公司）的记录
- 排除 2023-01-01 前已注销（`status=CAN`）的记录

同时生成 `merchant_name`（主名，同名冲突时追加 ABN）和 `keywords`（所有别名拼接），输出 `data/filtered.csv`。
...
将 `data/filtered.csv` 与已有的 `data/kb_internal.csv` 按 `match_key` 合并：
- **新增**：新 match_key → 加入知识库
- **更新**：已有 match_key，keywords 变化 → 标记为待清洗
- **消失**：旧知识库有但新报文无 → 标记为 `GONE`
- **不变**：keywords 无变化 → 保留已清洗数据

输出 `data/kb_internal.csv` 和 `data/changelog.csv`。
...
停用词集合定义在 `config.py`，包含 200+ 澳洲城市/区名、商业通用词、支付通道词等。

### categorize — 规则分类

```bash
python scripts/categorize.py
python scripts/categorize.py --input data/my_kb.csv
```

基于关键词匹配将商户归入 24 个类别。分类规则定义在 `category_rules.py`，采用计分制——匹配到某类别关键词越多，分数越高。已填充的分类不会被低置信度的新匹配覆盖。

**分类类别**：Automotive · Department Stores · Dining Out · Donations · Education · Entertainment · Fees · Financial Institutions · Gambling · Groceries · Gyms and other memberships · Health · Home Improvement · Information · Insurance · Personal Care · Pet Care · Rent · Retail · Subscription TV · Telecommunications · Transport · Travel · Utilities
