...
class MerchantCategoryPromptConfig:
    ...
    def build_batch_user_prompt(self, items: list[dict[str, str]]) -> str:
        ...
        return (
            ...
            "Prefer assigning one allowed category for ordinary consumer-facing merchants when the evidence points to a single most likely business type. "
            "A perfect match is not required; use the closest sensible enum category when one category is reasonably stronger than the others. "
            "Use an empty string only when evidence is too weak, conflicting, non-consumer-facing, or not represented by the enum.\n"
